 ...
 */
export async function generateReportPdf(data, indicadores, estadisticas, riesgos) {
  // Load background and logo images if available
  ...
  try {
    ...
    console.warn('Could not load background/logo images – proceeding without decorations.', e);
  }
  ...
  // Info chips: secretaria / direccion / unidad
  ...
  if (indicadores.length > 0) {
    ...
    const rows = Math.ceil(indicadores.length / 2);
    y += rows * 14 + 6;
  }

  // ========================= TWO-COLUMN: ESTADÍSTICAS =========================
  if (estadisticas.length > 0) {
    ...
    estadisticas.forEach((stat, i) => {
      ...
      pdf.text(label.substring(0, 28), xPos + 3, yPos + 4);
      pdf.setTextColor(30, 30, 30);
      pdf.setFontSize(12);
      pdf.setFont('helvetica', 'bold');
  ...
  if (riesgos.length > 0) {
    ...
    riesgos.forEach((riesgo, i) => {
      ...
      pdf.setFontSize(7);
      pdf.setFont('helvetica', 'bold');
      const cat = riesgo.cat || 'RIESGO';
      pdf.text(cat.substring(0, 20), xPos + 3, yPos + 4);
      pdf.setTextColor(100, 40, 40);
      pdf.setFontSize(6);
      pdf.setFont('helvetica', 'normal');
      const title = riesgo.title || '';
      pdf.text(title.substring(0, 50), xPos + 3, yPos + 9);
  ...
  // ========================= FINALIZE =========================
  // Update footer page counts across all pages
